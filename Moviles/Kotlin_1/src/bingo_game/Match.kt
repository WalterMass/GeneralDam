...
class Match : Playable<PlayerBingo> {
    ...
    override fun showResults(players: ArrayList<PlayerBingo>) {
        ...
        val ranking : MutableMap<Int, PlayerBingo> = rankingAddedPlayers.associateBy {
            it.score
        } as MutableMap<Int, PlayerBingo>
    ...
    fun pullWinningPlayers(ranking: MutableMap<Int, PlayerBingo>): Map <Int, PlayerBingo>{
            ...
            lines.forEach { line ->
                if (line.isNotBlank()){
                    ...
                    if (atributs.size >= 3) {
                        ...
                        val p = PlayerBingo(id, userName, key)
    ...
    fun saveWinningPlayers(addedWinners: MutableMap<Int, PlayerBingo>){
        val directory = File("src/files_bingo")
        val file = File(directory, "players.txt")
